...
class AsyncClientImpl extends PagiClient implements IEventListener
{
     ...
     * @var \PHPAMI\Client\IClient
     */
    private $pamiClient;
    /**
     * The event that originated this async agi request.
     * @var \PHPAMI\Message\Event\AsyncAGIEvent
     */
    ...
    {
        if ($event instanceof \PHPAMI\Message\Event\AsyncAGIEvent) {
    ...
    protected function send($text)
    {
        ...
        $action = new \PHPAMI\Message\Action\AGIAction($this->channel, $text, $this->lastCommandId);
     ...
     *
     * pamiClient => The PHPAMI client that will be used to run this async client.
     *
